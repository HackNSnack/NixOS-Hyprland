...
in
{
  imports = [
    ...
    ../../modules/services/tailscale-protonvpn.nix # Tailscale+ProtonVPN coexistence (NM dispatcher)
  ...
  # Tailscale (Tier 2) — private mesh so you can reach this host's services
  # away from home. Tailscale is network-level: every listening service on
  # the host is reachable from authenticated tailnet devices at the host's
  # 100.x.y.z tailnet IP. Which ports actually arrive is gated by the NixOS
  # firewall below. Chosen policy: open ports individually (explicit,
  # least-privilege). Jellyfin :8096 is already in allowedTCPPorts via its
  # openFirewall. Add future services' ports there as you build them, e.g.
  # Ollama :11434 on the desktop.
  services.tailscale = {
    enable = true;
    openFirewall = true; # opens UDP 41641 (tailscale daemon) only; NOT public
  };
